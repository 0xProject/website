...
2. Hosting and distributing orders
...
In addition to relayer hosting, distribution can also be achieved via Mesh, a peer-to-peer network built specifically for sharing 0x orders. For more information, see [0x.org/mesh](0x.org/mesh).

A 0x order is a data packet that contains an [order's details](https://0x.org/docs/guides/v3-specification#order-message-format) together with a [valid signature](https://0x.org/docs/guides/v3-specification#signature-types) (e.g. a valid elliptic curve signature generated from the traders Ethereum address) of it's contents.
...
There are two dominant relayer strategies in existence today: [matching](https://0x.org/docs/guides/relayer-strategies#matching) and [open orderbook](https://0x.org/docs/guides/relayer-strategies#open-orderbook). Open orderbook relayers host orders that anyone can fill by directly sending a transaction (with the order details and desired fill amount) to the Ethereum blockchain. [Mesh](https://0x.org/mesh), despite not being a centralized relayer, also falls into the open orderbook category, as it only distributes orders that can be filled by anyone. Using this model, the order in which transactions are filled is determined by the same algorithm used by miners to decide which transactions to include in the next block (more on this later).
...
For a set of scenarios showcasing the above steps, as well as allowance setting, take a look at the [0x Starter Project](https://github.com/0xProject/0x-starter-project).
...
Whereas posting orders, to a relayer or to Mesh, represents a "push" distribution model, there is an alternative "pull" model as well, known as Request for Quote (RFQ).  This is a feature of the [0x API](/docs/api), which implements not just the Standard Relayer API, but also additional resources for aggregating liquidity from both 0x and non-0x sources. For RFQ, the 0x API is configured with a list of market makers, and whenever a trader requests a quote for the best available pricing across all sources, the API will contact those market makers, which may respond with a 0x order representing their "quote."  If that order outcompetes pricing from other sources, it will be given to the API client, who may fill it just as they would fill any other 0x order.  For more details, see [RFQ-T in the 0x API](/docs/guides/rfqt-in-the-0x-api).
