...
**contract-wrappers** is a package that contains a friendly wrapper around interacting directly with on-chain contracts.
**utils** Contains helpers like BigNumber, a JavaScript library for arbitrary-precision decimal and non-decimal arithmetic.
**connect** is a Standard Relayer API client implementation that allows you to easily query any Standard Relayer API endpoint.
**web3-wrapper** is a package for interacting with an Ethereum node, retrieving account information. This is an optional package and you can choose to use alternatives like Web3.js or Ethers.js.
**order-utils** is a package where we have our deployed contract addresses for the various networks. This also includes an address for a deployed Wrapped Ether contract.
...
If you wish for the call to block until the transaction has been successfully mined, you may use `awaitTransactionSuccessAsync`.
...
- **takerAddress** : Ethereum address of our **Taker**. `NULL_ADDRESS` means the order can be filled by anyone.
...
- **makerFeeAmount**: How many base units the **Maker** will pay in **makerFeeAssetData** as a fee to the **feeRecipientAddress**.
- **takerFeeAmount** : How many base units the **Taker** will pay in **takerFeeAssetData** as a fee to the **feeRecipientAddress**.
...
While all of these tasks were performed using Ganache and a local standard relayer api compliant HTTP server, you can start using [@0x/connect](https://www.npmjs.com/package/@0x/connect) in conjunction with 0x API: https://api.0x.org/sra/ for executing trades on the main Ethereum network. For more information on how to use our libraries, go [here](https://0x.org/docs/tools).
