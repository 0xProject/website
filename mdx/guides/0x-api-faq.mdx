...
The 0x core team holds itself to a high security standard with all the smart contracts written; 0x protocol’s contracts are open source and extensively tested by 0x core team’s internal protocol team and by external auditors.

The [**Exchange**](https://0x.org/docs/guides/0x-cheat-sheet) contract, because they are the main canonical interface of the 0x protocol, is versioned, and governed by ZRX token holders. Because of its critical place in 0x infrastructure, the 0x core team employs a number of external audits to ensure the safe and intended usage of the 0x contract.

The [**Forwarder**](https://0x.org/docs/guides/0x-cheat-sheet) contract is (as we call it in the 0x ecosystem) an extension contract. Namely, it is an optional “middleware” contract that users can opt in/out of using when swapping. Because of its optionality and minimized attack surface (no approval of user funds needed to utilize Forwarder), the *Forwarder* contract is internally audited and constantly improved upon by the 0x core team.
...
The `price` field provides developers a sense of what the BEST price you can receive, denominated in `sellToken` for one `buyToken`. The `gauranteedPrice` is the price that developers can expect in a WORST case scenario where all the fall back orders are utilized over the better priced orders.
