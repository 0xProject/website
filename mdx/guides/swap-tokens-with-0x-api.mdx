...
(async () => {
    ...
    const response = await fetch(
        'https://api.0x.org/swap/v0/quote?buyToken=DAI&sellToken=ETH&buyAmount=1000000000000000000 (https://api.0x.org/swap/v0/quote?buyToken=DAI&sellToken=WETH&buyAmount=1000)'
        );
...
sources like Oasis, Uniswap, and Kyber. 
...
Because this is a swap between ERC20 tokens, you need to give the [0x ERC20Proxy](https://etherscan.io/address/0x95e6f48254609a6ee006f7d493c8e5fb97094cef) approval to move your tokens for you. Otherwise, the token swap works exactly the same way as before.
...
The API will also perform a `eth_call` to catch any reverts that would occur if the `takerAddress` attempts to swap the tokens. An HTTP response with status `400` will be returned if the eth_call results in a revert (ie. the swap fails), along with reasons for the revert.
...
* If `sellToken` is a ERC20 token, (not ETH), the `takerAddress` will need to approve [0x ERC20Proxy](https://etherscan.io/address/0x95e6f48254609a6ee006f7d493c8e5fb97094cef).
...
* Pass a few entries in the quote to a smart contract to perform the swap on-chain.

## Advanced features

### Changing the gasPrice parameter

In 0x v3, a new protocol fee (paid in ETH or WETH) dependent on gas price is introduced. Provide the optional param `gasPrice` so 0x API can calculate an accurate `protocolFee` for the quote. If no `gasPrice` parameter is provided, the API will utilize the ‘fast’ gasPrice provided by ethgasstation. If you decide to use a lower `gasPrice` than the one provided to the API, the protocol will refund the excess ETH paid.
...
(async () => {
    ...
    if (response.ok) {
        // set value to zero so protocol fee will be pa
...
* Refer to our [API spec](/docs/api)for detailed documentation on the 0x API.
