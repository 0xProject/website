...
Because this is a swap between ERC20 tokens, we need to give the [0x ERC20Proxy](./v3-specification#assetproxy) smart contract an approval to move your tokens for you. Otherwise, the token swap works exactly as before:
...
(async () => {
    ...
    const USDCcontract = new ERC20TokenContract(USDCaddress, provider);
...
The API performs an `eth_call` under the hood to catch any reverts that would occur if the `takerAddress` attempts to swap the tokens. An HTTP response with status 400 will be returned if the `eth_call` results in a revert (ie. the proposed swap fails), along with reasons for the revert.
