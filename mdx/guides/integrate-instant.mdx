...
As we now support multiple [token transfer proxies](https://0x.org/docs/guides/v3-specification#assetproxy), the identifier of which proxy to use for the token transfer must be encoded, along with the token information. Each proxy in 0x has a unique identifier. If you're using `@0x/contract-wrappers` there are helper methods for this [encoding](https://0x.org/docs/tools/contract-wrappers#encodeerc20assetdata) and [decoding](https://0x.org/docs/tools/contract-wrappers#decodeerc20assetdata).
...
For more information see [the Asset Proxy](https://0x.org/docs/guides/v3-specification#erc20proxy) section of the v3 spec and the [Ethereum ABI Spec](https://solidity.readthedocs.io/en/develop/abi-spec.html).
