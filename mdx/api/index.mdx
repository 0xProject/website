...
*   Capture a percentage of the trade. Set `feeType=FIXED` and `buyTokenPercentageFee` to set the percentage of the `buyAmount` that should be attributed to the `feeRecipient` address.
*   Capture the positive slippage of a trade. Positive slippage is the difference between the quoted price and the realized price when the realized price is higher. Set `feeType=POSITIVE_SLIPPAGE` and the `feeRecipient` to activate this feature.

### Fees Misc.
*   Adding a fee will result in an increased of gas for the taker.
*   When setting `feeType=POSITIVE_SLIPPAGE` and when the trade benefit positive slippage, 0x API will only send the positive slippage funds to `feeRecipient` if the amount is higher than the gas cost to do that operation.
*   When using fees (`feeType=FIXED` or `feeType=POSITIVE_SLIPPAGE`), 0x API won't be able to use our highly gas optimized UniswapV2 Router (https://0xprotocol.readthedocs.io/en/latest/advanced/uniswap.html).
...
| `feeType`               | (Optional) Type of the affiliate fee (`POSITIVE_SLIPPAGE`, `FIXED`). Use `POSITIVE_SLIPPAGE` for sending any positive slippage (difference between quoted price and realized price when realized price is higher) to `feeRecipient` or `FIXED` to use `buyTokenPercentageFee`).                                                                                                 |
...
| `feeRecipient`          | (Optional) The ETH address that should receive affiliate fees (specified with `buyTokenPercentageFee` or with `feeType=POSITIVE_SLIPPAGE`).                                                                                                                                                                                                                                                                   |
