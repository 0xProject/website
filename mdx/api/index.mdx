...
Before being committed to a trade, a requestor should send quote requests to the `/price` endpoint. This endpoint will compute the best price available and return it to the caller. Once a trader has decided they wish to perform the trade, a final request should be made to the `/quote` endpoint.
...
| `buyAmount`          | (Optional) The amount of `buyToken` (in `buyToken` base units) you want to receive.                                                                                                                                      |
| `slippagePercentage` | (Optional) The maximum acceptable slippage in % of the `buyToken` amount if `sellAmount` is provided, the maximum acceptable slippage in % of the `sellAmount` amount if `buyAmount` is provided. This parameter will change over time with market conditions. |
| `takerAddress`       | (Required) The address which will fill the quote.          |
| `excludedSources`    | (Optional) Liquidity sources (`Eth2Dai`, `Uniswap`, `Kyber`, `0x`, `LiquidityProvider` etc) that will not be included in the provided quote. Ex: `excludedSources=Uniswap,Kyber,Eth2Dai`. See [here](https://github.com/0xProject/0x-monorepo/blob/development/packages/asset-swapper/src/utils/market_operation_utils/types.ts#L27) for a full list of sources  |
...
When a trader has shown an intent to complete a trade, calling this `/quote` endpoint will return a [0x transaction](https://github.com/0xProject/0x-protocol-specification/blob/master/v3/v3-specification.md#transactions) hash for them to sign, confirming to the details of the trade. 0x transactions is a 0x-native feature that enables meta-transactions by separating the taker of a trade included in an Ethereum transaction, from the party submitting the Ethereum transaction. Once the trader has signed the 0x transaction hash, the integrator should send it to the `/fill` endpoint. 
...
## GET /meta_transaction/v0/fill

The endpoint accepts a signed 0x transaction hash and will do one of two things: (1) submit the Ethereum transaction executing the trade encoded in the 0x transaction OR (2) return an unsigned Ethereum transaction that executes the 0x transaction. (1) occurs IF the request is sent with a whitelisted `0x-api-key` header (reach out to us about this possibility) and otherwise returns the unsigned Ethereum transaction for either the end user or integrator/dApp to sign and broadcast. 
...
| Query Param          | Description                                                                                                                                                                                                         |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `signature`          | (Required) 0x transaction hex-encoded signature.                                                                                                      |
| `zeroExTransaction`           | (Required) The 0x transaction struct corresponding to the supplied signature.                                                                                                                                                 |
