...
| `sellAmount`         | (Optional) The amount of `sellToken` (in `sellToken` units) you want to send. If `sellAmount` is provided, `buyAmount` will be ignored.                                                                             |
| `buyAmount`          | (Optional) The amount of `buyToken` (in `buyToken` units) you want. When both are provided, `sellAmount` takes precedence over this argument. Either a `sellAmount` or `buyAmount` is required.                     |
