...
RFQ-T liquidity can only be accessed if the request supplies a taker address, a permissioned API key via header `0x-api-key`, and the query parameter `intentOnFilling=true`.  For more details see the guide [Understanding RFQ-T and the 0x API](/docs/guides/understanding-rfqt-0x-api).  Also, ensure that `Native` sources have not been excluded.

An RFQ-T quote can be identified by its inclusion of one or more orders with a `takerAddress` matching the `takerAddress` that was supplied in the request.  However, the lack of such an order in a quote does not indicate that RFQ-T liquidity was not considered; it only indicates that RFQ-T liquidity was not competitive enough to be included in the quote.

<HttpCall fontSize="16px" path="/swap/v0/quote?buyToken=WETH&sellToken=ETH&buyAmount=10000000&takerAddress=0xffffffffffffffffffffffffffffffffffffffff" />
