...
## Fees
There are two ways to capture affiliate fees from a trade.
*   Capture a percentage of the trade. Set `feeType=buy-token-percentage` and `buyTokenPercentageFee` to set the percentage of the `buyAmount` that should be attributed to the `feeRecipient` address.
*   Capture the positive slippage of a trade. Positive slippage is the difference between the quoted price and the realized price when the realized price is higher. Set `feeType=positive-slippage` and the `feeRecipient` to activate this feature.

### Fees Misc.
*   Adding a fee will result in an increased of gas for the taker.
*   When setting `feeType=positive-slippage` and when the trade benefit positive slippage, 0x API will only send the positive slippage funds to `feeRecipient` only if the positive slippage funds are higher than the gas cost to do that operation.
*   When using fees (`feeType=buyTokenPercentageFee` or `feeType=`positive-slippage`), 0x API won't be able to use our highly gas optimized UniswapV2 Router (https://0xprotocol.readthedocs.io/en/latest/advanced/uniswap.html).
...
| `feeType`               | (Optional) Type of the affiliate fee (`positive-slippage`, `buy-token-percentage`). Use `positive-slippage` for sending any positive slippage (difference between quoted price and realized price when realized price is higher) to `feeRecipient` or `buy-token-percentage` to use `buyTokenPercentageFee`).                                                                                                 |
...
| `feeRecipient`          | (Optional) The ETH address that should receive affiliate fees (specified with `buyTokenPercentageFee` or with `feeType=positive-slippage`).                                                                                                                                                                                                                                                                   |
...
<HttpCall fontSize="16px" path="/swap/v1/quote?sellToken=WETH&buyToken=DAI&sellAmount=1000000000000000000&takerAddress=0xff...fff&intentOnFilling=true" />
...
0x API SRA uses [0x Mesh](https://0x-org.gitbook.io/mesh/) as the underlying orderbook.
...
<HttpCall path="/sra/v4/orders?makerToken=0xf47261b...756cc2" />
...
<HttpCall verb="POST" path="/sra/v4/order" >
...
<HttpCall verb="POST" path="/sra/v4/orders" >
...
<HttpCall path="/sra/v4/order/0xa1e06a9...eaa29e1a6" />
...
<HttpCall fontSize="14px" path="/sra/v4/orderbook?quoteToken=0xc02aaa...756cc2&baseToken=0xa0b869...06eb48" />
...
<HttpCall verb="POST" path="/sra/v4/order_config">
...
<HttpCall path="/sra/v4/fee_recipients" />
