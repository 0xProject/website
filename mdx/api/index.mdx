...
Before a trader has committed to a trade, a requestor should send quote requests to the `/price` endpoint. This endpoint will compute the best price available and return it to the caller. Once a trader has decided they wish to perform the trade, a final request should be made by the requestor to the `/quote` endpoint.
