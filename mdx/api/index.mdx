...
| `intentOnFilling`    | (Required when `includedSources=RFQT`) When requesting RFQT liquidity, `intentOnFilling=true` tells our market makers that the integrator is committed to completing the trade |
| `skipValidation`     | (Optional, requires `takerAddress`) When set to `true`, the 0x API does not simulate the trade on-chain                                                                        |
