...
| `makerToken`             | The address The ERC20 token the maker is selling and the maker is selling to the taker.                                                                                                                                                                                                                                                |
| `takerToken`             | The address of the ERC20 token the taker is selling and the taker is selling to the maker                                                                                                                                                                                                                                              |
...
| `verifyingContract`      | Address of the contract where the transaction should be sent.                                                                                                                                                                                                                                                                          |
| `chainId`                | The ID of the Ethereum chain where the `verifyingContract` is located.                                                                                                                                                                                                                                                                 |
| `signature`              | A [JSON object structure](https://protocol.0x.org/en/latest/basics/orders.html#how-to-sign) with the signature of the fields above using the private key of `maker`.                                                                                                                                                                   |
...
| Field                   | Description                                                                                                                                                                                                                                                                                                                                        |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `makerToken`            | (Optional) The address of the ERC20 token the maker is selling and the maker is selling to the taker. Example `0xe41d2489571d322189246dafa5ebde1f4699f498`
| `takerToken`            | (Optional) The address of the ERC20 token the taker is selling and the taker is selling to the maker. Example `0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2`
| `makerAmount`           | (Optional) The amount of makerToken being sold by the maker.
| `takerAmount`           | (Optional) The amount of takerToken being sold by the taker.
| `maker`                 | (Optional) The address of the maker, and signer
| `taker`                 | (Optional) Allowed taker address
| `pool`                  | (Optional) The staking pool to attribute the 0x protocol fee from this order. Set to zero to attribute to the default pool, not owned by anyone. TODO FIX THIS
| `feeRecipient`          | (Optional) Recipient of maker token or taker token fees (if non-zero)
  ...
  "records": [
    {
      "metaData": {
        ...
        "remainingFillableTakerAssetAmount": "2000000000000000000000"
      },
      "order": {
        "chainId": 1,
        "expiry": "1614954899",
    ...
    "records": [
      {
        ...
        "metaData": {
          ...
          "remainingFillableTakerAssetAmount": "10000000",
...
| Field                     | Description                                                                                                                                                                                                                                                                                                                            |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `feeRecipient`            | The entity that will receive any fees stipulated by the order. This is typically used to incentivize off-chain order relay.                                                                                                                                                                                                            |
| `sender`                  | An advanced field that doesn't need to be set. It allows the maker to enforce that the order flow through some additional logic before it can be filled (e.g., a KYC whitelist) -- more on the ability to extend 0x later.                                                                                                             |
...
| Kovan   | wss://kovan.api.0x.org/sra/v4 |
  ...
  "payload": [
    {
      ...
      "metaData": {
        ...
        "remainingFillableTakerAssetAmount": "1000000000000000",
        "state": "ADDED"
      }
    }
  ],
